...
```
The dataset we chose has many factors and only a couple of continuous variables. From the plot above we can see that Experience vs Age has a nice linear model and Wage vs Education also has a bit of a linear model. Another interesting thing about this plot is that the distribution between men and women with education is about the same. We can also see that wage for men is a bit higher compared to women this might imply the existance of a wage gap. We are going explore more indepth into these models and using real life knowledge to see if we are able to explain the reasoning for a wage gap between male and female.
...
summary(mod);summary(mod1) #see that we have low r^2 value and nothing is interesting lets try doing added variable plots
```
...
As we can see from the avPlots function WAGE~EXPERIENCE and WAGE~EDUCATION both have nice linear fits which shows that adding EXPERIENCE into our plot is a good idea. 
...
p6 + geom_point() + facet_wrap(wages$SEX~wages$OCCUPATION) + geom_smooth(method="lm",aes(color=SEX), data=wages)
#all this shows is that this model is a good fit for occupation for male and female
#with this model though how does one check the wage gap. Maybe just check the summary and see that sex female make less
#Also with the occupation model you can compare if male and female make more or less in certain occupations
```

#Adding Union and Marriage to see if the models still fit well with occupation
```{r}
mod5 = lm(log(WAGE)~tranEXPERIENCE + EDUCATION + SEX + UNION + OCCUPATION, data=wages)
summary(mod5)
#So adding Union does increase the $R^2$ value so that means we should add it to our model

mod6 = lm(log(WAGE)~tranEXPERIENCE + EDUCATION + SEX + UNION + OCCUPATION + MARR, data=wages)
summary(mod6);summary(mod5)
#This shows that mod6 R^2 value and variance did not really change so Marriage does not have a influence in this model. 
```

#Checking Model 5 and seeing the wage gap in different occupations
```{r}
#Do a principle componet and bring down the dimensions. 
prcomp(mod5)
```
